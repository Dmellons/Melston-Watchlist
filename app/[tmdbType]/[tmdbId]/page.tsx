...
const DetailPage = async ({ params }: DetailPageProps) => {
    ...
    // @ts-ignore
    let data: TMDBApiMovieDetail | TMDBApiTvDetail = {}
    
    if (tmdbType === 'movie') {
        data = await response.json() as TMDBApiMovieDetail;
    }

    if (tmdbType === 'tv') {
        data = await response.json() as TMDBApiTvDetail;
    }
                ...
                <div className="space-y-6">
                    ...
                    {tmdbType === 'tv' &&
                        <>
                            ...
                            <p>Next Air Date: {data.next_episode_to_air_date ? new Date(data.next_episode_to_air).toLocalDateString() : <span className='font-bol'>Series Complete</span>}</p>
                    ...
                    <p>Runtime: {data.runtime ?? data.episode_run_time} minutes</p>
                </div>
            </div>
